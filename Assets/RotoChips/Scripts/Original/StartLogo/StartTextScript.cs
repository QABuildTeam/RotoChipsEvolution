...
public class StartTextScript : MonoBehaviour {
	...
	void Start () {
        GetComponent<Text>().text = LocalizationManager.instance.GetLocalizedValue(loadingTextKey);
    ...
    IEnumerator makeButtonInteractable(){
        while (!StressImageCreator.instance.hasFinalImage(0))
        {
            yield return null;
        }
        GetComponent<Text>().text = LocalizationManager.instance.GetLocalizedValue(tapToStartTextKey);
