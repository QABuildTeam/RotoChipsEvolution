	...
	IEnumerator preloadImages() {
		...
		for (int i = 0; i < imgCount; i++) {
			...
			if (i == 0) {
				...
				string finalImageName = StressImageCreator.instance.stressedFinalImageFile(currentGalleryLevel);
				// the image may be not ready yet, so wait for the image generatorß
				while (!StressImageCreator.instance.hasFinalImage(currentGalleryLevel))
				{
